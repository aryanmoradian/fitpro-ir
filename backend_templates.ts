...
create table profiles (
  ...
  -- Athletic Background
  sport_level text, -- 'beginner', 'intermediate', 'professional'
  specialization text, -- 'bodybuilding', 'fitness', etc.
  
  -- SUBSCRIPTION & TRIAL LOGIC
  -- Ensure training_start_date is populated on creation for trial calculation
  training_start_date date default current_date, 
  
  goal_type text, -- 'fatLoss', 'muscleGain', etc.
  
  -- Settings
  privacy_settings jsonb default '{"isPublic": false, "hideWeight": false, "hideBodyFat": false, "aiRecommendationVisibility": true}',

  -- Subscription
  subscription_tier text default 'free',
  subscription_status text default 'active', -- Default active for Trial period
...
returns trigger as $$
begin
  insert into public.profiles (id, email, first_name, last_name, role, training_start_date)
  values (
    new.id, 
    new.email, 
    ...
    current_date -- START 30 DAY TRIAL
  );
  return new;
end;
...
-- 2. Athlete History (Metric Time-Series)
create table athlete_history (
  ...
  weight float,
  body_fat float,
  source text default 'manual', -- 'manual', 'smart_scale', 'body_scan'
  created_at timestamptz default now()
);
alter table athlete_history enable row level security;
create policy "Users manage own history" on athlete_history for all using (auth.uid() = user_id);
...
-- 8. Transactions Log (Advanced Financials - Optional Extension)
...
-- 9. Athlete Activity (Consolidated Data for Analytics Export)
...
-- 11. Body Scans (AI Progress Tracker)
create table body_scans (
  ...
  weight numeric,
  photos jsonb not null, -- Stores URLs for {front, back, sideLeft, sideRight}
  notes text,
  ai_analysis jsonb, -- Stores the cached analysis result from Gemini
  created_at timestamptz default now()
);
alter table body_scans enable row level security;
create policy "Users manage own scans" on body_scans for all using (auth.uid() = user_id);
`;
