...
const AdvancedAnalyticsDashboard: React.FC<{
  ...
  setCurrentView: (view: AppView) => void;
    ...
    // Safe defensive check - Prevents "Cannot read properties of undefined (reading 'length')"
    const safeLogs = Array.isArray(logs) ? logs : [];
    const safeMetrics = Array.isArray(profile?.metricsHistory) ? profile.metricsHistory : [];

    useEffect(() => {
        if (safeLogs.length > 1) { // Need at least 2 logs for a trend
            setIsAnalyzing(true);
  ...
  const hasTrainingData = safeLogs.length > 0;
  const hasNutritionData = safeLogs.some(l => l.consumedMacros && l.consumedMacros.calories > 0);
  const hasBodyCompData = safeMetrics.length > 1;
  const hasRecoveryData = safeLogs.some(l => l.sleepHours && l.sleepHours > 0);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            ...
            <WidgetCard title="شاخص پیشرفت تمرینی" icon={TrendingUp} hasData={hasTrainingData} emptyState={{ message: "برای فعال‌سازی این شاخص، تمرینات خود را ثبت کنید.", ctaText: "ثبت تمرین امروز", onCtaClick: () => setCurrentView(AppView.TRACKER) }} color="blue">
                <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={safeLogs} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
            ...
            <WidgetCard title="تحلیل ترکیب بدنی" icon={BarChart2} hasData={!!hasBodyCompData} emptyState={{ message: "برای مشاهده روند، حداقل دو بار اطلاعات بدن خود را ثبت کنید.", ctaText: "ثبت وضعیت جدید", onCtaClick: () => setCurrentView(AppView.BODY_ANALYSIS) }} color="purple">
                <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={safeMetrics} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                        <CartesianGrid strokeDasharray="3 3" stroke="#374151" />
                        <XAxis dataKey="date" stroke="#9ca3af" fontSize={10}/>
  ...
  return (
    <div className="h-full flex items-center justify-center animate-in fade-in p-4">
      <div className="text-center bg-black/30 p-8 rounded-2xl border border-white/10 shadow-xl max-w-5xl">
        ...
        <h2 className="text-3xl font-bold text-white mb-3">راهنمای جامع تحلیل زیستی</h2>
