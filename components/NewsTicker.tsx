...
const NewsTicker: React.FC<TickerProps> = ({ logs, profile, metrics, setCurrentView }) => {
  const tickerItems = useMemo(() => {
    ...
    // Ensure we don't access indices if array is empty
    const lastLog = safeLogs.length > 0 ? safeLogs[safeLogs.length - 1] : null;
    const lastMetric = safeMetrics.length > 0 ? safeMetrics[safeMetrics.length - 1] : null;
    const prevMetric = safeMetrics.length > 1 ? safeMetrics[safeMetrics.length - 2] : null;
    ...
    if (profile?.advancedHealth?.recoveryIndex) {
        ...
        const colorClass = recovery > 75 ? 'text-green-400' : recovery > 50 ? 'text-yellow-400' : 'text-red-400';
         items.push(
            <TickerItem key="recovery" onClick={() => setCurrentView(AppView.BIOLOGICAL_ANALYSIS)} className="bg-indigo-900/50 text-indigo-300">
                <HeartPulse size={16} />
    ...
    if (profile?.advancedHealth?.vo2Max) {
         items.push(
            <TickerItem key="vo2max" onClick={() => setCurrentView(AppView.BIOLOGICAL_ANALYSIS)} className="bg-cyan-900/50 text-cyan-300">
                <Gauge size={16} />
